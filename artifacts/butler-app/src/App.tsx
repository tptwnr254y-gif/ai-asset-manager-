...
type AppContextValue = {
  user: User | null; butler: Butler | null; schedule: ScheduleItem[]; logs: LifeLog[]; goals: Goal[]; memories: Memory[];
  ...
  setDark: (value: boolean) => void; logout: () => void;
};
...
function AuthPage() {
  ...
  const submit = (event: FormEvent) => {
    ...
    if (!email.trim() || !email.includes('@') || (register && !name.trim())) {
      setMessage(register ? '이름과 이메일을 모두 정확히 입력해 주세요.' : '이메일 주소를 정확히 입력해 주세요.');
      return;
    }
    const user = { name: register ? name.trim() : email.split('@')[0], email: email.trim() };
    write('butler-session', user);
    setLocation('/butler');
  };
  return (
    <main className="min-h-[100dvh] grid lg:grid-cols-[1.06fr_.94fr] bg-[hsl(var(--background))]">
      <section className="hero-ink relative overflow-hidden px-7 py-10 sm:px-14 sm:py-14 lg:px-20 lg:py-20 flex flex-col justify-between min-h-[330px] lg:min-h-[100dvh]">
        <div className="relative z-10 flex items-center gap-3"><div className="w-9 h-9 rounded-full border border-[hsl(var(--accent)/.65)] grid place-items-center text-[hsl(var(--accent))]"><Crown size={17} /></div><span className="font-semibold tracking-[.2em] text-xs">버틀러 에이아이</span></div>
        <div className="relative z-10 max-w-xl mt-16 lg:mt-0">
          <p className="eyebrow text-[hsl(var(--accent))] mb-5">당신의 하루를 맡기는 방식</p>
          <h1 className="display-font text-5xl sm:text-7xl leading-[.95] font-semibold tracking-[-.04em]">계획보다 먼저,<br /><span className="text-[hsl(var(--accent))]">당신을 살피는</span> 집사.</h1>
          <p className="mt-7 max-w-md text-sm leading-7 text-[hsl(39 30% 92% / .7)]">말 몇 마디와 작은 기록만으로 오늘의 리듬을 정돈합니다. 조용히 곁에 있되, 결정은 언제나 당신의 것입니다.</p>
        </div>
      ...
      <section className="flex items-center px-6 py-12 sm:px-14 lg:px-20">
        <div className="w-full max-w-md mx-auto fade-up">
          <div className="mb-10"><p className="eyebrow mb-3">개인 생활 비서</p><h2 className="text-3xl font-semibold tracking-tight">{register ? '처음 뵙겠습니다.' : '다시 오셨군요.'}</h2><p className="mt-3 text-sm text-[hsl(var(--muted-foreground))]">{register ? '이름과 이메일로 나만의 공간을 열어 보세요.' : '오늘도 당신의 리듬부터 살피겠습니다.'}</p></div>
          <form onSubmit={submit} className="space-y-5">
            {register && <div><label className="label" htmlFor="auth-name">이름</label><input data-testid="input-auth-name" id="auth-name" className="field" value={name} onChange={e => setName(e.target.value)} placeholder="어떻게 불러드릴까요?" /></div>}
            ...
            <button data-testid="button-auth-submit" className="btn-primary w-full flex items-center justify-center gap-2" type="submit">{register ? '공간 열기' : '입장하기'}<ArrowRight size={17} /></button>
          </form>
          <div className="my-8 flex items-center gap-3 text-xs text-[hsl(var(--muted-foreground))]"><span className="h-px bg-[hsl(var(--border))] flex-1" />브라우저 데모 세션<span className="h-px bg-[hsl(var(--border))] flex-1" /></div>
          <button data-testid="button-auth-toggle" className="btn-quiet w-full" onClick={() => { setRegister(v => !v); setMessage(''); }}>{register ? '이미 계정이 있어요' : '처음 시작해 볼게요'}</button>
          <p className="mt-6 text-center text-xs text-[hsl(var(--muted-foreground))]">서버 연결 없이 이 브라우저에만 저장됩니다.</p>
        </div>
      </section>
    </main>
  );
  ...
  return (
    <main className="min-h-[100dvh] app-frame px-5 py-8 sm:px-10 sm:py-12">
      <div className="max-w-6xl mx-auto">
        <div className="flex items-center justify-between mb-16"><div className="flex items-center gap-3"><div className="w-9 h-9 rounded-full bg-[hsl(var(--primary))] text-[hsl(var(--accent))] grid place-items-center"><Crown size={17} /></div><span className="font-bold tracking-[.18em] text-xs">버틀러 에이아이</span></div><span className="text-sm text-[hsl(var(--muted-foreground))]">{user.name}님</span></div>
...
function AppShell({ children }: { children: ReactNode }) {
  const { user, butler, dark, setDark, logout } = useApp();
  const [location, setLocation] = useLocation();
  if (!user || !butler) return null;
  const active = (href: string) => href === '/app' ? location === '/app' : location.startsWith(href);
  return <div className="app-frame noise min-h-[100dvh] md:grid md:grid-cols-[236px_1fr]">
    <aside className="shell-sidebar hidden md:flex flex-col px-5 py-7 sticky top-0 h-[100dvh]">
      <Link href="/app" data-testid="link-brand-home" className="flex items-center gap-3 px-2"><div className="w-9 h-9 rounded-full border border-[hsl(var(--accent)/.55)] text-[hsl(var(--accent))] grid place-items-center"><Crown size={17} /></div><span className="font-bold tracking-[.18em] text-xs">버틀러 에이아이</span></Link>
    ...
    <div className="shell-main min-w-0">
      <header className="h-[72px] px-[18px] md:px-12 flex items-center justify-between border-b border-[hsl(var(--border)/.7)]">
        <div className="md:hidden flex items-center gap-2"><div className="w-8 h-8 rounded-full bg-[hsl(var(--primary))] text-[hsl(var(--accent))] grid place-items-center"><Crown size={15} /></div><span className="font-bold tracking-[.15em] text-[10px]">버틀러 에이아이</span></div>
...
function SettingsPage() {
  const { user, butler, dark, setDark, memories, setMemories } = useApp();
...
function ChatPage() {
  const { butler } = useApp();
  const [messages, setMessages] = useState<ChatMessage[]>(() => read('butler-chat', [{ id: 'welcome', role: 'butler', text: `${butler?.name || '집사'}입니다. 오늘은 어떤 일을 맡겨 볼까요?`, createdAt: now() }]));
  const [text, setText] = useState('');
  useEffect(() => write('butler-chat', messages), [messages]);
  const send = (event: FormEvent) => { event.preventDefault(); if (!text.trim()) return; const userText = text.trim(); setMessages(prev => [...prev, { id: makeId(), role: 'user', text: userText, createdAt: now() }, { id: makeId(), role: 'butler', text: preparedButlerReply(userText, butler!), createdAt: now() }]); setText(''); };
  return <AppShell><div className="page-width"><div className="mb-8 fade-up"><p className="eyebrow mb-3">대화의 방</p><h1 className="display-font text-5xl font-semibold leading-none">집사에게 묻기</h1><p className="mt-4 text-sm text-[hsl(var(--muted-foreground))]">현재는 로컬 데모 응답으로 동작합니다. 실제 인공지능 연결은 준비 중입니다.</p></div><div className="max-w-3xl mx-auto surface rounded-3xl overflow-hidden"><div className="p-5 sm:p-7 border-b border-[hsl(var(--border))] flex items-center gap-3"><Portrait butler={butler!} small /><div><p className="font-semibold">{butler?.name} 집사</p><p className="text-xs text-[hsl(var(--muted-foreground))] mt-1">로컬 데모 대화 · 외부 전송 없음</p></div><button data-testid="button-reset-chat" className="icon-btn ml-auto" onClick={() => setMessages([{ id: makeId(), role: 'butler', text: `${butler?.name}입니다. 대화를 새로 시작할게요.`, createdAt: now() }])} aria-label="대화 초기화"><RotateCcw size={15} /></button></div><div className="min-h-[360px] max-h-[52vh] overflow-y-auto p-5 sm:p-7 space-y-5">{messages.map(message => <div data-testid={`message-chat-${message.id}`} key={message.id} className={`flex gap-3 ${message.role === 'user' ? 'justify-end' : ''}`}>{message.role === 'butler' && <Portrait butler={butler!} small />}<div className={`max-w-[80%] rounded-2xl px-4 py-3 text-sm leading-6 ${message.role === 'user' ? 'bg-[hsl(var(--primary))] text-[hsl(var(--primary-foreground))] rounded-br-sm' : 'bg-[hsl(var(--secondary))] rounded-bl-sm'}`}>{message.text}</div></div>)}</div><form onSubmit={send} className="p-4 sm:p-5 border-t border-[hsl(var(--border))] flex gap-2"><input data-testid="input-chat-message" className="field" value={text} onChange={e => setText(e.target.value)} placeholder="오늘의 마음이나 할 일을 말해 주세요" /><button data-testid="button-send-chat" className="btn-primary px-4" type="submit"><ArrowRight size={17} /></button></form></div><div className="max-w-3xl mx-auto mt-4 flex flex-wrap gap-2">{['오늘 일정을 정리해 줘', '조금 피곤해', '이번 주 목표를 세우고 싶어'].map(prompt => <button data-testid={`button-prompt-${prompt.slice(0, 3)}`} key={prompt} onClick={() => setText(prompt)} className="btn-quiet text-xs">{prompt}</button>)}</div></div></AppShell>;
}
...
function Guard({ children }: { children: ReactNode }) {
  const { user, butler } = useApp();
  const [, setLocation] = useLocation();
  useEffect(() => { if (!user) setLocation('/'); else if (!butler) setLocation('/butler'); }, [user, butler, setLocation]);
  return user && butler ? <>{children}</> : <div className="min-h-[100dvh] grid place-items-center app-frame"><div className="skeleton w-40 h-4" /></div>;
}
...
function Entry() {
  ...
  return user ? <div className="min-h-[100dvh] grid place-items-center app-frame"><div className="skeleton w-40 h-4" /></div> : <AuthPage />;
}
...
function App() {
  const [user, setUser] = useState<User | null>(() => read('butler-session', null));
  const [butlerId, setButlerId] = useState<string | null>(() => read('butler-selected', null));
  const [schedule, setScheduleState] = useState<ScheduleItem[]>(() => read('butler-schedule', starterSchedule()));
  const [logs, setLogs] = useState<LifeLog[]>(() => read('butler-logs', [starterLog()]));
  const [goals, setGoalsState] = useState<Goal[]>(() => read('butler-goals', starterGoals));
  const [memories, setMemoriesState] = useState<Memory[]>(() => read('butler-memories', starterMemories));
  const [dark, setDarkState] = useState<boolean>(() => read('butler-dark', false));
  const butler = useMemo(() => BUTLERS.find(item => item.id === butlerId) || null, [butlerId]);
  useEffect(() => { document.documentElement.classList.toggle('dark', dark); write('butler-dark', dark); }, [dark]);
  const setButler = (item: Butler) => { setButlerId(item.id); write('butler-selected', item.id); };
  const setSchedule = (items: ScheduleItem[]) => { setScheduleState(items); write('butler-schedule', items); };
  const saveLog = (log: LifeLog) => { const next = [...logs.filter(item => item.date !== log.date), log]; setLogs(next); write('butler-logs', next); };
  const setGoals = (items: Goal[]) => { setGoalsState(items); write('butler-goals', items); };
  const setMemories = (items: Memory[]) => { setMemoriesState(items); write('butler-memories', items); };
  const setDark = (value: boolean) => setDarkState(value);
  const logout = () => { setUser(null); setButlerId(null); localStorage.removeItem('butler-session'); localStorage.removeItem('butler-selected'); };
  const value: AppContextValue = { user, butler, schedule, logs, goals, memories, dark, setButler, setSchedule, saveLog, setGoals, setMemories, setDark, logout };
